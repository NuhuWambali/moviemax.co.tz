...
<head>
    ...
    @include('partials.seo', ['seoTitle' => ($series->title ?? 'MOVIEMAX') . ' - MOVIEMAX', 'seoDescription' => ($series->title ?? '') . ' (' . ($series->release_year ?? '') . '): ' . Illuminate\Support\Str::limit($series->description ?? 'Watch and download this series on MOVIEMAX.', 160), 'seoImagePath' => $series->poster_path ? url($series->poster_path) : asset('images/posters/dummy-poster.png'), 'seoType' => 'video.tv_show', 'seoJsonLd' => [['@type' => 'TVSeries', 'name' => $series->title ?? '', 'description' => $series->description ?? '', 'image' => $series->poster_path ? url($series->poster_path) : asset('images/posters/dummy-poster.png'), 'startDate' => $series->release_year ?? '', 'genre' => $series->genre ?? '', 'url' => url('/series/' . $series->id)], ['@type' => 'BreadcrumbList', 'itemListElement' => [['@type' => 'ListItem', 'position' => 1, 'name' => 'Home', 'item' => url('/')], ['@type' => 'ListItem', 'position' => 2, 'name' => 'TV Series', 'item' => url('/series')], ['@type' => 'ListItem', 'position' => 3, 'name' => $series->title ?? '', 'item' => url('/series/' . $series->id)]]]]])
