...
<head>
    ...
    @include('partials.seo', ['seoTitle' => ($trailer->title ?? '') . ' – Official Trailer | MovieMax', 'seoDescription' => 'Watch the official trailer for ' . ($trailer->title ?? 'this film') . ' in HD on MovieMax. Discover ' . ($trailer->title ?? 'this') . ' release date, synopsis and more.', 'seoImagePath' => $trailer->thumb_url ?? '', 'seoType' => 'video.other', 'seoJsonLd' => [['@type' => 'VideoObject', 'name' => $trailer->title ?? '', 'description' => $trailer->description ?? '', 'thumbnailUrl' => $trailer->thumb_url ?? '', 'uploadDate' => ($trailer->created_at ?? now())->toIso8601String(), 'url' => url('/trailers/' . $trailer->slug)], ['@type' => 'BreadcrumbList', 'itemListElement' => [['@type' => 'ListItem', 'position' => 1, 'name' => 'Home', 'item' => url('/')], ['@type' => 'ListItem', 'position' => 2, 'name' => 'Trailers', 'item' => url('/trailers')], ['@type' => 'ListItem', 'position' => 3, 'name' => $trailer->title ?? '', 'item' => url('/trailers/' . $trailer->slug)]]]]])
    <style>
        :root {
            --bg-deep: #0a0d12;
            --bg-surface: #0f141b;
