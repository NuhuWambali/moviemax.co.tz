...
<head>
    ...
    @include('partials.seo', ['seoTitle' => ($movie->title ?? 'MOVIEMAX') . ' - MOVIEMAX', 'seoDescription' => ($movie->title ?? '') . ' (' . ($movie->release_year ?? '') . '): ' . Illuminate\Support\Str::limit($movie->description ?? 'Watch and download this movie in HD on MOVIEMAX.', 160), 'seoImagePath' => $movie->poster_url ?? asset('images/posters/dummy-poster.png'), 'seoType' => 'video.movie', 'seoJsonLd' => [['@type' => 'Movie', 'name' => $movie->title ?? '', 'description' => $movie->description ?? '', 'image' => $movie->poster_url ?? asset('images/posters/dummy-poster.png'), 'datePublished' => $movie->release_year ?? '', 'genre' => $movie->genre ?? '', 'url' => url('/movies/' . ($movie->slug ?? $movie->id))], ['@type' => 'BreadcrumbList', 'itemListElement' => [['@type' => 'ListItem', 'position' => 1, 'name' => 'Home', 'item' => url('/')], ['@type' => 'ListItem', 'position' => 2, 'name' => 'Movies', 'item' => url('/movies')], ['@type' => 'ListItem', 'position' => 3, 'name' => $movie->title ?? '', 'item' => url('/movies/' . ($movie->slug ?? $movie->id))]]]]])
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes glowPulse {
            0%, 100% { box-shadow: 0 0 20px rgba(59, 130, 246, 0.25), 0 0 40px rgba(59, 130, 246, 0.1); }
            50% { box-shadow: 0 0 30px rgba(59, 130, 246, 0.4), 0 0 60px rgba(59, 130, 246, 0.2); }
        }
        @keyframes posterGlow {
            0%, 100% { box-shadow: 0 25px 60px rgba(0,0,0,0.6), 0 0 40px rgba(59, 130, 246, 0.1), 0 0 80px rgba(59, 130, 246, 0.05); }
            50% { box-shadow: 0 25px 60px rgba(0,0,0,0.6), 0 0 60px rgba(59, 130, 246, 0.2), 0 0 100px rgba(59, 130, 246, 0.1); }
        }
        @keyframes shimmer {
...
.dropdown-content a:hover {
        ...
        .trailer-container {
            ...
            overflow: hidden;
            background: #000;
            box-shadow: 0 0 60px rgba(59, 130, 246, 0.18), 0 30px 80px rgba(0, 0, 0, 0.8);
        ...
        .related-card:hover {
            ...
            box-shadow: 0 20px 40px -15px rgba(59, 130, 246, 0.2), 0 0 60px -20px rgba(59, 130, 246, 0.15);
        }
    ...
    <script>
        ...
        function showDownloadOptions() {
            Swal.fire({
                ...
                html: `
                    <div style="text-align: left; margin: 1rem 0;">
                        <p><i class="fas fa-check-circle" style="color: #3b82f6;"></i> High quality video</p>
                        <p><i class="fas fa-check-circle" style="color: #3b82f6;"></i> Lifetime access</p>
                        <p><i class="fas fa-check-circle" style="color: #3b82f6;"></i> Download on any device</p>
                    </div>
                `
            }).then((result) => {
