        ...
        <form method="POST" action="{{ route('admin.hero-slides.update', $heroSlide) }}" enctype="multipart/form-data" style="background: #121217; padding: 2rem; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05);">
            ...
            <div class="form-group">
                ...
                <select name="link_type" id="linkType" class="form-control" onchange="toggleLinkSelects()">
                    ...
                    <option value="trailer" {{ old('link_type', $heroSlide->link_type) === 'trailer' ? 'selected' : '' }}>Link to a Trailer</option>
                </select>
                <div id="trailerSelect" style="display: none; margin-top: 0.75rem;">
                    ...
                    <select name="link_id" id="trailerSelectElt" class="form-control">
                        @foreach($trailers as $trailer)
                            <option value="{{ $trailer->id }}" {{ old('link_type', $heroSlide->link_type) === 'trailer' && (int) old('link_id', $heroSlide->link_id) === $trailer->id ? 'selected' : '' }}>{{ $trailer->title }}</option>
                        @endforeach
                    </select>
                </div>
